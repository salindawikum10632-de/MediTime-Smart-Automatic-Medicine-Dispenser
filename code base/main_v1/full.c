...
// -------------------- TIME SET VARIABLES --------------------
uint8_t set_hour, set_min;

enum { MODE_NORMAL, MODE_SET_TIME, MODE_SET_ALARM1, MODE_SET_ALARM2 };
    ...
    while (1) {
        ...
        if (button_pressed(&PINB, BTN_SET, &last_btn_set)) {
            if (mode == MODE_SET_TIME) {
                ds1302_set_time(set_hour, set_min, 0);
            }
            mode++;
            ...
            if (mode == MODE_SET_TIME) {
                uint8_t dummy_sec;
                ds1302_get_time(&set_hour, &set_min, &dummy_sec);
            }
        }
        ...
        if (mode == MODE_SET_TIME) {
            if (button_pressed(&PINC, BTN_UP, &last_btn_up)) {
                if (edit_field == 0) set_hour = (set_hour + 1) % 24;
                else set_min = (set_min + 1) % 60;
            }
            if (button_pressed(&PIND, BTN_DOWN, &last_btn_down)) {
                if (edit_field == 0) set_hour = (set_hour + 23) % 24;
                else set_min = (set_min + 59) % 60;
            }
        }
        ...
        if (mode == MODE_NORMAL)
            ...
            lcd_printf("Set T %s %02d:%02d  ", (edit_field == 0) ? "Hr" : "Mn", set_hour, set_min);
